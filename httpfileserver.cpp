...
int HandleSpecialFile(char *args, SimpleSocket* s, char *req_headers, bool *special_file_js);
...
int HandleCommand(SimpleSocket* s, char* cmdbuf, int len)
{
    ...
    case COMMAND_GET:
        ...
        /* special files */
        if(HandleSpecialFile(args, s, headersbuf, &special_file_js))
        {
            err = -1; /* disconnect */
            break;
        }
        ...
        else if (err == 1)
        {
            ...
            else
            {
                if(special_file_js)
                {
                    CreateDirFile(DirOfPath(filepath));
                    SendDirectoryJS(s, filepath);
                }
                else
                {
...
int HandlePOST(SimpleSocket* s, char* path, char *headersbuf)
{
    ...
    *ptr2 = '\r';
    ...
    if(strEndsWith(path, "/upload_file"))
    {
        ...
        if(strstr(gPermissions, "R")) replaceFile = true;
...
int HandleSpecialFile(char *args, SimpleSocket* s, char *req_headers, bool *special_file_js)
{
    ...
    else if(strEndsWith(args, "/files.js"))
    {
        *special_file_js = true;
        return 0;
    }

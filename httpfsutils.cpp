...
void LookupContentTypeByExt(char* str, /* at most 64 chars in length */ char* dest)
{
    ...
    ptr = buf;
    for(;*extension != 0 && *extension != '\r' && *extension != '\n';extension++) {
...
int SendDirectoryHelper(SimpleSocket* s, char* path, int size_only, int javascript)
{
    ...
    if(!size_only)
    {
        size = SendDirectoryHelper(s, path, 1, javascript);
        if(size < 0)
        {
            delete file;
            return -1;
    ...
    header = "<A HREF='../'>Up one level</A><BR>";
    if(javascript) header = "var fileList = [";
    strcat(htmlbuffer, header);
    ...
    while(!file->eof())
    {
        ...
        if(javascript)
        {
           ...
           strcat(htmlbuffer, "', 'thumbnail':'");
           strcat(htmlbuffer, "thumbs/"); strcat(htmlbuffer, file_path);
           strcat(htmlbuffer, "', 'is_dir':");
           strcat(htmlbuffer, isdir? "true": "false");
           strcat(htmlbuffer, "}");
           goto next_file; 
        }

        Escape(file_path, sizeof(file_path)-1, allowedCharsHTTP);
...
{
    return SendDirectoryHelper(s, path, 0, 0);
}

int SendDirectoryJS(SimpleSocket* s, char* path)
{
    return SendDirectoryHelper(s, path, 0, 1);
}
